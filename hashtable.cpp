/*
Why does redis implement their own STL? 
throughput and latency.
throughput is an average case problem that can solved with
sharding and read-only replicas
latency is much harder to solve. what happens if an
insertion triggers O(n) resize operation?

Most STL libraries are optimized throughput, not latency
*/
#include <assert.h>
...
static void h_init(HTab *htab, size_t n) {
    assert(n > 0 && ((n - 1) & n) == 0);
    htab->tab = (HNode **)calloc(n, sizeof(HNode *)); // array of array slots
    htab->mask = n - 1;
    htab->size = 0;
}
...
static void h_insert(HTab *htab, HNode *node) {
    size_t pos = node->hcode & htab->mask; // get the position with bitmask
    HNode *next = htab->tab[pos]; // temporarily store the current head of the chain (will be next)
    node->next = next; // point the new head to the current head (attach to the chain)
    htab->tab[pos] = node; // update the head of the chain (insert)
    htab->size++;
}
...
static HNode **h_lookup(HTab *htab, HNode *key, bool (*eq)(HNode *, HNode *)) {
    ...
    size_t pos = key->hcode & htab->mask; // get the position of the hashcode tab[pos]
    HNode **from = &htab->tab[pos];     // incoming pointer in the chain
